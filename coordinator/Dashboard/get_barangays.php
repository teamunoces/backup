...
switch ($action) {
    ...
    case 'demographics':
        ...
        $barangayTables = ["aclan","amontay","ataatahon","barangay1","barangay2","barangay3",
                           "barangay4","barangay5","barangay6","barangay7","cubicubi","culit",
                           "jaguimitan","kinajabangan","punta","santaana","talisay","triangulo","camagong"];
        $data = [];
        foreach ($barangayTables as $table) {
        ...
        while ($row = $tablesResult->fetch_array()) {
            ...
            // Check if the table has a 'status' column first
            $colCheck = $conn->query("SHOW COLUMNS FROM `$tableName` LIKE 'status'");
