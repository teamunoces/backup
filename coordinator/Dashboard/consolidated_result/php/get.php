            ...
            foreach($data['waste'] as $i=>&$row){
              ...
              // --------TABLE 14. Types of Training Ranked --------
                            ...
                            $data['training_need'] = $needData;
                                ...
                                // Lower average = higher priority (1 is highest, 5 is lowest)
                                ...
                                $types = [
                                    "Baking and cooking for Mothers",
                                    "Computer Literacy for Out-of-School Youth",
                                    "Crime prevention ug disaster management para sa mga Barangay Tanod ug opisyales"
                                ];
                                ...
                                // 1️⃣ Count predefined trainings
                                foreach ($types as $t) {
                                    $stmt = $surveyPdo->prepare("
                                        SELECT COUNT(*) FROM `$barangay`
                                ...
                                foreach ($data['dtr'] as $index => $row) {
                                    if ($previousTotal !== null && $row['total'] < $previousTotal) {
                                        $rank = $index + 1;
                                    }
                                    
                                    $data['dtr'][$index]['rank'] = $rank;
                                    $previousTotal = $row['total'];
                                }

                    // -------- TABLE 17. DESIRED RELIGIOUS ACTIVITIES   --------
                        $types = [
                            "lecture kabahin sa mga sakramento sa simbahan",
                            "Mga grupo sa pagtuon sa bibliya",
                            "Espiritwal nga retreats ug recollections",
                            "Mga programa para sa pagpalig-on sa pagtuo sa kabatanonan ug pamilya",
                            "Grupo sa pag-ampo o intercessory ministries"
                        ];
                        ...
                        // 1️⃣ Count predefined activities
                        foreach ($types as $t) {
                            $stmt = $surveyPdo->prepare("
                                SELECT COUNT(*) FROM `$barangay`
                        ...
                        foreach ($data['religious_act'] as $index => $row) {
                            if ($previousTotal !== null && $row['total'] < $previousTotal) {
                                $rank = $index + 1;
                            }
                            
                            $data['religious_act'][$index]['rank'] = $rank;
                            $previousTotal = $row['total'];
                        }
                  // --------   TABLE 18.  IMPORTANT RELIGIOUS ACTIVITY    --------
                                                                        $types = [
                                                                            "Pag-ampo ug meditasyon",
                                                                            "Pagtuon sa Bibliya o relihiyosong edukasyon",
                                                                            "Pagsalmot sa mga pagsimba",
                                                                            "Espiritwal nga tambag ug giya",
                                                                            "Pag-alagad ug tabang sa uban"  // Removed duplicate
                                                                        ];
                                                                        ...
                                                                        foreach ($data['ira'] as $index => $row) {
                                                                            if ($previousTotal !== null && $row['total'] < $previousTotal) {
                                                                                $rank = $index + 1;
                                                                            }
                                                                            
                                                                            $data['ira'][$index]['rank'] = $rank;
                                                                            $previousTotal = $row['total'];
                                                                        }

                 // -------- TABLE 19    SPIRITUAL GROWTH    --------
                                    $types = [
                                        "Pagsalmot sa retreats o recollections",
                                        "Apil sa gagmay nga faith-sharing groups",
                                        "Moapil sa youth o family-focused nga relihiyosong events"
                                    ];
                                    ...
                                    foreach ($data['growth'] as $index => $row) {
                                        if ($previousTotal !== null && $row['total'] < $previousTotal) {
                                            $rank = $index + 1;
                                        }
                                        
                                        $data['growth'][$index]['rank'] = $rank;
                                        $previousTotal = $row['total'];
                                    }
                 ...
                 // -------------------- TABLE 20.  KA PILA SILA MAG AMPO    -------------
                                    ...
                                    // Assign ranks properly (without using reference)
                                    $rank = 1;
                                    $previousTotal = null;

                                    foreach ($data['freq'] as $index => $row) {
                                        if ($previousTotal !== null && $row['total'] < $previousTotal) {
                                            $rank = $index + 1;
                                        }
                                        
                                        // Don't use reference, just access the array directly
                                        $data['freq'][$index]['rank'] = $rank;
                                        $previousTotal = $row['total'];
                                    }
