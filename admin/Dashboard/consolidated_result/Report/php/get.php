...
try {
    ...
    // Add year condition if not 'all'
    if ($year !== 'all' && is_numeric($year)) {
        // For survey database
        $yearCondition = " AND YEAR(created_at) = ?";
        $params[] = $year;
        
        // For demographics database - also using created_at
        $demoYearCondition = " AND YEAR(created_at) = ?";
        $demoParams[] = $year;
    }
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $typedArr[] = $row['Other_religion'];
    }

    if (!empty($typedArr)) {
        $data['religion'][] = [
            "religion" => "Others",
            "total" => count($typedArr),
            "other" => count($typedArr),
            "display" => "Others (" . implode(",", $typedArr) . ")",
            "percentage" => 0
        ];
    }
    ...
    while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
        $data['helps'][] = [
            "helps" => $row['barangay_needs'],
            "total" => (int)$row['total_count'],
            "percentage" => $total ? round($row['total_count']/$total*100,2) : 0
        ];
    }
